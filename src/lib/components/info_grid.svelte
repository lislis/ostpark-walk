<div class="gridlike">

    <h2>
        <span  class="a11y-hidden">Karte</span>
        <img src="/icons/home.svg" width="40" height="40" alt=""></h2>
    <div>
        <p>
        In der Karten-Ansicht hast du die Übersicht über den gesamten Ostfriedhof. Die verschiedenen Symbole wie Lautsprecher, Rechteck etc. markieren die Orte, an denen du Inhalte entdecken kannst. Wenn du nah genug bist, um auf sie zuzugreifen, beginnen sie zu hüpfen!<br>
        <strong>Wichtig:</strong> Bitte gib der App die Berechtigung, auf deinen Standort zuzugreifen!
        </p>
    </div>

    <h2>
        <span  class="a11y-hidden">GPS</span>
        <img src="/icons/shoes-marker.svg" width="40" height="40" alt=""></h2>
    <div>
        <p>
        Im Zentrum der Karte siehst du deine Position auf der Karte.
        </p>
    </div>

    <h2>
        <span  class="a11y-hidden">Audio</span>
        <img src="/icons/audio.svg" width="40" height="40" alt=""></h2>
    <div>
        <p>
        Das Lautsprecher-Symbol steht für Medien, die man hören kann.
       Wenn du in einem Umkreis von 10 Meter des markierten Ortes bist, fängt die Markierung an zu hüpfen, ein Player erscheint und du kannst das Audio abspielen.
       Wenn du auf "Transkript anzeigen" klickst, bekommst du das ganze als Text angezeigt. 

        </p>
    </div>

    <h2>
        <span  class="a11y-hidden">3D</span>
        <img src="/icons/3d.svg" width="40" height="40" alt=""></h2>
    <div>
        <p>
        Bei den Würfeln kannst du nicht nur etwas hören, sondern zusätzlich durch die Kamera deines Handys etwas sehen. Bitte klick dafür im Player auf "Kameraansicht".<br>
        <strong>Wichtig: </strong> Bitte gib der App die Berechtigung, auf deine Kamera zuzugreifen!
        </p>
    </div>


    <h2>
        <span  class="a11y-hidden">Kamera</span>
        <img src="/icons/camera.svg" width="40" height="40" alt=""></h2>
    <div>
        <p>
        In der Kamera-Ansicht kannst du die Welt um dich herum durch die Kamera deines Smartphones sehen.
        Hier haben sich einige Dinge versteckt, die du mit deinen eigenen Augen nicht wahrnehmen kannst.
        </p>
    </div>

    <h2>
        <span  class="a11y-hidden">Fragen</span>
        <img src="/icons/questions.svg" width="40" height="40" alt=""></h2>
    <div>
        <p>
        In der Kamera-Ansicht kannst du die Welt um dich herum durch die Kamera deines Smartphones sehen.
        Hier haben sich einige Dinge versteckt, die du mit deinen eigenen Augen nicht wahrnehmen kannst.
        </p>
    </div>

</div>
